...
Peridot is an open, research-oriented web application for exploring humanistic data through maps, networks, timelines, charts, advanced search, exports, and evidence dossiers. Its mature first use case is correspondence data based on the creator's dissertation research, and its active import system also includes a person-centered genealogy profile alongside increasingly general support for point/site data, chart-first time series, generic evidence records, and multi-sheet workbooks. Underneath those workflows, Peridot uses a canonical normalized research model. The generalized upload architecture is now authoritative for single-table and workbook imports, including repeatable places and relationships plus a substantially richer temporal model that preserves multiple dates/periods, partial and approximate dates, open intervals, source text, and researcher-supplied temporal notes.
...
6. Map fields through the progressive workflow: review the source table, define any relationship participants, add and name any dates or periods, add any place roles, choose Evidence fields with Include/Ignore, and double-check the assignments on Review.
...
The generalized single-table and workbook mapping architecture is now authoritative through canonical normalization and the active runtime bridge. Compatibility projections still exist for legacy consumers and should be retired only after a repository-wide audit proves that their behavior has been replaced without loss. The current generalized Network path also still has known follow-up work around consuming all generalized relationships consistently.

Temporal parsing and Timeline now consume canonical temporal assertions, but some legacy date helpers remain as compatibility paths for sample/demo or unmigrated consumers. A later cleanup should retire `parseHistoricalDate()` / `parsedDate` only after all remaining consumers have migrated. Additional optional Timeline filters for temporal structure—such as approximate or partial dates—remain deferred.
